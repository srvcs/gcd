use srvcs_gcd::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-gcd listening");
    let deps = Deps {
        modulo_url: cfg.modulo_url.clone(),
        iszero_url: cfg.iszero_url.clone(),
    };
